...
#### 8. タイムスタンプの実装（実際の発言時刻の記録）

**現状の問題:**
- 現在、タイムライン表示では**話者識別ボタンを押した時刻**が全ての発言に表示されます
- 実際に発言した時刻は記録されておらず、後から見返した時に「どの発言がいつ行われたか」が分かりません

**改善案:**
- **音声認識時にタイムスタンプを埋め込む（推奨）**: 音声認識の最終結果が確定した時に、テキストにタイムスタンプを埋め込む（例：`[14:32] 河野です。...`）
  - Gemini APIに渡しても時刻情報が保持される
  - 実装が比較的シンプル
  - 関連ファイル: `minutes/js/speechRecognition.js`（音声認識時）、`minutes/js/timelineRenderer.js`（表示時）

- **発言ごとのタイムスタンプを記録**: 音声認識時に発言と時刻のペアを記録し、話者識別後にマッピング
  - より正確だが、Gemini APIが発言を分割・統合する可能性があるため対応付けが困難
  - 実装が複雑

- **推定タイムスタンプ**: 音声認識の開始・終了時刻から発言数で等分して推定
  - 実装は簡単だが不正確

**技術的な詳細:**
- 現在の実装: `minutes/js/timelineRenderer.js:145-148` でレンダリング時の`new Date()`を使用
- データ構造: `{ utterances: [{ name, text, timestamp }, ...] }` にtimestampフィールドを追加する必要がある
...
**理由:**
- 現在のフロントエンド構成（純粋なHTML/CSS/JavaScript）が最適
- さくらサーバーの環境制約により、Pythonバックエンドの運用が困難
- レスポンスタイムの悪化により、リアルタイム音声認識の要件を満たせない

**推奨アプローチ:**
- 現在のフロントエンドアーキテクチャを維持
- 必要に応じて、Cloudflare Workers等のエッジコンピューティングを検討
- バックエンドが必要な場合は、さくらサーバー以外のホスティングサービスを検討
